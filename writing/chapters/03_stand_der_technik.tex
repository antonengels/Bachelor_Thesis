...
\gls{RL}-Agenten haben Schwierigkeiten, die Übergangsdynamik von Umgebungen so intuitiv wie Menschen zu verstehen und Vorwissen effektiv anzuwenden \cite[vgl.~S.~1]{chap1_Wexler22}. 
Die Literatur beschreibt diesen Engpass als zentralen Grund dafür, dass \gls{RL}- und \gls{IL}-Ansätze vorhandenes Expertenwissen in komplexen Domänen nur unzureichend ausnutzen \cite[S.~5042]{chap1_Silva19}. 
Für die weitere Argumentation wird daher auf die in Abschnitt \ref{sec:problemstellung} eingeführte Initialisierungsproblematik verwiesen.
...
Wie bereits in Abschnitt \ref{sec:problemstellung} beschrieben, nutzen reine \gls{RL}-Ansätze vorhandenes Expertenwissen ohne geeignete Initialisierung nur unzureichend \cite[vgl.~S.~5042]{chap1_Silva19}. 
...
Die initiale Policy $\pi_\theta^{BC}$ sichert dem System eine solide Grundkompetenz, welche die für das \gls{RL} typische Sample-Ineffizienz und das Risiko von sicherheitskritischen Aktionen während der initialen Explorationsphase drastisch reduziert.
...
Um diesen Distribution Shift zu korrigieren, setzt die Wissenschaft in Simulationen häufig auf interaktive Algorithmen wie \gls{DAgger}. 
,,Interaktive \gls{IL}-Algorithmen wie \gls{DAgger}... nutzen eine Reduktion auf No-Regret-Online-Learning und demonstrieren, dass sie unter bestimmten Bedingungen erfolgreich eine Policy erlernen können, die den Experten imitiert'' \cite[S.~1]{chap3_Cha21}. 
\gls{DAgger} fordert einen "Teacher in the Loop", der das Modell kontinuierlich während der Fahrt korrigiert, sobald es in einen unbekannten Zustand gerät. 

Für den realen Bahnbetrieb und historische Offline-Datensätze birgt dieser Diskurs jedoch ein massives Dilemma. 
Solche Korrekturen erfordern Echtzeitinteraktion: ,,Diese interaktiven \gls{IL}-Algorithmen können den Covariate Shift jedoch nicht nachweislich vermeiden, wenn der Experte nicht wiederherstellbar ist... Online-Interaktionen sind oft kostspielig und für reale Anwendungen verboten, bei denen eine aktive Trial-and-Error-Exploration... unsicher sein könnte'' \cite[S.~1]{chap3_Cha21}. 
Es mangelt an Systemen, die die Offline-Limitierung aufbrechen. Für reale Schienenanwendungen ist dieser Ansatz jedoch ungeeignet, da im realen Fahrbetrieb kein mathematisches Oracle existiert, das während des Zuglaufs in Millisekunden-Intervallen kontinuierlich optimale Steuerungssignale berechnen und bereitstellen kann. 
...
Um der daraus resultierenden Modell-Degradation zu begegnen, setzt die Forschung vor allem auf interaktive Verfahren wie \gls{DAgger}, die den Lernprozess online an die Expertenverteilung heranführen. 
Entsprechend kommen in der Literatur rekurrente Architekturen wie \glspl{LSTM} und \glspl{GRU} zum Einsatz \cite[S.~2]{chap3_Lyu26}.

Darüber hinaus gilt in der Literatur der Konsens, dass Modelle strikt daran gehindert werden müssen, streckenspezifische Rauschmuster zu memorisieren. 
Um der Leistungsdegradation (Degradation in Warm-Start \gls{RL}) und dem Overfitting bei verrauschten Daten entgegenzuwirken, müssen Updates der Policy durch Konfidenzmaße oder Abstandsmetriken an die Expertenverteilung stark regularisiert bzw. eingeschränkt (Constrained Learning) werden \cite[vgl.~S.~1f.~u.~S.~6]{chap1_Wexler22}. 
...
Die Abkehr von diesen Systemen hin zum \acrlongshort{DRL} bleibt - wie in Abschnitt \ref{sec:3.1.3} und Abschnitt \ref{sec:imitation_learning} diskutiert - durch Sample-Ineffizienz, Sparse Rewards und eingeschränkte Initialisierungsmöglichkeiten limitiert. 
Etablierte Imitation-Learning-Ansätze adressieren diese Hürde zwar prinzipiell, verlassen sich in der aktuellen Literatur jedoch primär auf saubere, synthetische Simulatordaten oder fordern unrealistische, interaktive Korrekturen durch ein Experten-Oracle während des Trainingsprozesses (\gls{DAgger}-Algorithmus).

Genau an diesem Schnittpunkt manifestiert sich die zentrale Forschungslücke: 
Es mangelt an fundierten Konzepten, die die in Abschnitt \ref{sec:problemstellung} beschriebenen Initialisierungshürden zukünftiger \gls{RL}-Agenten ausschließlich durch Offline \gls{BC} auf Basis realer, historischer Betriebsdaten lösen. 
...
Die daraus gewonnene Policy dient als essenzielles Fundament und stabile Ausgangspolicy, um den hochdimensionalen Suchraum für spätere belohnungsbasierte Reinforcement-Learning-Optimierungen effektiv einzugrenzen.